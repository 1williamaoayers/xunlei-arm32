...
var (
	SYNO_PLATFORM = getSynoPlatform()                                                                                                        //平台
	SYNO_MODEL    = getSynoModel()                                                                                                           //平台
	OS_VERSION    = SYNO_PLATFORM + " dsm " + SYNOPKG_DSM_VERSION_MAJOR + "." + SYNOPKG_DSM_VERSION_MINOR + "-" + SYNOPKG_DSM_VERSION_BUILD //系统版本
)

// getSynoPlatform 返回平台标识
func getSynoPlatform() string {
	if runtime.GOARCH == "amd64" {
		return "geminilake"
	}
	return "rtd1296" // arm64 和 arm 都使用这个
}

// getSynoModel 返回设备型号
func getSynoModel() string {
	if runtime.GOARCH == "amd64" {
		return "DS920+"
	}
	return "DS220j" // arm64 和 arm 都使用这个
}

// getXunleiCLI 返回迅雷 CLI 二进制路径
func getXunleiCLI() string {
	base := SYNOPKG_PKGDEST + "/bin/bin/xunlei-pan-cli-launcher."
	if runtime.GOARCH == "arm" {
		// ARM32: 使用 ARM64 二进制，通过 QEMU 运行
		return base + "arm64"
	}
	return base + runtime.GOARCH
}

// getExecutor 返回执行器路径
// ARM32 使用 QEMU 用户态模拟运行 ARM64 二进制
...
// getExecutorArgs 返回执行器参数
func getExecutorArgs() []string {
	if runtime.GOARCH == "arm" {
		return []string{getXunleiCLI()}
	}
	return nil
}
...
func (d *Daemon) directRun(ctx context.Context) (err error) {
	...
	// ARM32 使用 QEMU 执行器运行 ARM64 二进制
	executorArgs := append(getExecutorArgs(), args...)
	err = cmd.Run(ctx, getExecutor(), executorArgs, cmd.RunOption{
